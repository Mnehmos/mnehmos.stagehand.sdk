...
function internalDepsFor(ws, ledger, owners) {
  ...
  // Edges the feature graph cannot express: two packages owned by one feature, where one imports
  // the other. Declared in workspaces.json because a feature dependency cannot be self-referential.
  for (const extra of ws.additional_internal_dependencies ?? []) deps.add(extra);
  return [...deps].sort();
}
...
/** Relative path from one workspace directory to another, for a tsconfig `references` entry. */
...
function renderTsconfig(ws, internalDeps, workspaces) {
  const depth = ws.dir.split('/').length;
  const up = '../'.repeat(depth);
  // Project references so `tsc -b` builds dependencies first and resolves each cross-package import
  // through the referenced project rather than through a fresh compile of the same sources.
  // `composite` is what makes a project referenceable at all, and it is also why this config emits
  // rather than checking only: a referenced project must have declarations to point at.
  const references = internalDeps
    .map((name) => workspaces.find((candidate) => candidate.name === name))
    .filter((dep) => dep !== undefined)
    .map((dep) => ({ path: relativeDir(ws.dir, dep.dir) }));
...
/** The build solution: an empty file that exists only to name the projects and their order. */
function renderBuildSolution(workspaces) {
  const doc = {
    $comment: 'Generated by tools/workspaces.mjs --write. `tsc -b tsconfig.build.json` builds every package in dependency order and emits dist/.',
    files: [],
    references: workspaces.map((ws) => ({ path: ws.dir })),
  };
  return JSON.stringify(doc, null, 2) + '\n';
}
...
function expectedFiles(workspaces, ledger) {
  ...
  for (const ws of workspaces) {
    ...
    structural.set(`${ws.dir}/tsconfig.json`, renderTsconfig(ws, internalDeps, workspaces));
  ...
  structural.set('tsconfig.build.json', renderBuildSolution(workspaces));
